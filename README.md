...
灵梦御所资源下载工具：输入资源代码（如 `R4191`、`S5271`），查询该资源的 MEGA / 磁力 / 百度网盘等下载链接，并把 MEGA 资源直接下载到本机。

> 数据源为云端预抓的静态资源库，查询代码即查即用、开箱即用。
...
本工具查询代码走**云端预抓数据**（`data.json`），该数据由云端定时自动同步。你只需下载 MEGA 文件时保持本机运行。
...
下载目录默认在你用户主目录下的默认下载目录（名称用环境变量 `REIMU_SAVE_DIR` 指定后创建）。

云端数据源默认由 `WAREHOUSE_DATA_URL` 环境变量指定，未设置时使用内置默认源。
...
```bash
export MEGA_PROXY=http://127.0.0.1:1234   # 你的代理地址
```

## 常见问题

**Q: 提示"云端资源加载失败"？**
启动时从云端数据源拉取数据失败。检查网络能否访问该地址，或稍后重试。
（也可手动在页面点「刷新数据」重新拉取。）
...
**Q: 某代码提示未找到？**
云端库只覆盖部分代码。可能是较新的投稿尚未同步（等云端每天自动更新），或该代码不在收录范围。
